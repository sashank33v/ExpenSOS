...
# Patch input.css
input_css_path = os.path.join(FRONTEND_DIR, 'input.css')
...
for tmpl in ['login.html', 'register.html']:
    path = os.path.join(FRONTEND_DIR, 'templates', tmpl)
    exact_replace(path, 'glass-panel', 'auth-glass-panel')
    exact_replace(path, 'glass-input', 'auth-glass-input')
    exact_replace(path, 'btn-primary', 'auth-btn-primary')

# Patch base.html & auth_base.html
for filename in [ 'base.html', 'auth_base.html' ]:
